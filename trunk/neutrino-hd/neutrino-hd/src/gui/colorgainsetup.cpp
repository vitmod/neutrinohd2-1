...
#define COLORGAIN_SETUP_ICON_DESELECTED      	"volumeslider2"
#define COLORGAIN_SETUP_ICON_SELECTED 		"volumeslider2red"


CColorGainSetup::CColorGainSetup(const neutrino_locale_t Name, unsigned char * Value, CChangeObserver* Observer)
{
	...
	frameBuffer->setColorGain(* value);
}

int CColorGainSetup::exec(CMenuTarget * parent, const std::string &)
{
	neutrino_msg_t      msg;
			...
			case CRCInput::RC_vfddown:
				{
					if(selected<max)
					{
						paintSlider(x + 10, y + hheight, value, LOCALE_GTXALPHA_ALPHA1, COLORGAIN_SETUP_ICON_DESELECTED, false);

						selected++;

						if(selected == 0)
							paintSlider(x + 10, y + hheight, value, LOCALE_GTXALPHA_ALPHA1, COLORGAIN_SETUP_ICON_SELECTED, true );
					}
					break;
				}
...
void CColorGainSetup::paintSlider(const int x, const int y, const unsigned char * const spos, const neutrino_locale_t text, const char * const iconname, const bool selected) // UTF-8
{
	if (!spos)
		return;
