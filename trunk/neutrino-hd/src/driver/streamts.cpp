...
void streamts_main_thread(void * /*data*/)
{
	...
	printf("[streamts] Starting STREAM thread keeper, tid %ld\n", syscall(__NR_gettid));
...
void * streamts_live_thread(void *data)
{
	...
	*bp++ = 0;
	...
	cDemux * dmx = new cDemux( channel?channel->getFeIndex():0 );
