...
struct network_service
{
	...
	std::string cmd;
	std::string options;
	/*neutrino_locale_t*/std::string hint;
	std::string icon;
	int enabled;
};
...
{
	{ "FTP", "vsftpd", "", "FTPD", "", 0 },
	{ "Telnet", "telnetd", "-l/bin/login", "TELNETD", "", 0 },
	{ "DjMount", "djmount", "-o iocharset=utf8 /media/00upnp/", "DJMOUNT", "", 0 },
	{ "uShare", "ushare", "-D -n `cat /etc/hostname`", "USHARE", "", 0 },
	{ "xupnpd", "xupnpd", "", "XUPNPD", "", 0 },
};
...
{
	printf("Plugins: starting network services\n");
	
	CNetworkServiceSetup * networkServiceMenu = new CNetworkServiceSetup();
	
	networkServiceMenu->exec(NULL, "");
