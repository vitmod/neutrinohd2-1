...
#if defined HAVE_DBOX_HARDWARE || defined HAVE_COOL_HARDWARE || HAVE_TRIPLEDRAGON || HAVE_SPARK_HARDWARE || defined (PLATFORM_CUBEREVO_MINI2)
int GetRCCode()
{
	static int count = 0;
	//get code
	...
	if(read(rc, &ev, sizeof(ev)) == sizeof(ev))
	{
		if(ev.value)
		{
			...
			switch(ev.code)
			{
				...
				case 0x9e/*KEY_HOME*/:		rccode = RC_HOME;		break;
				//case 0x9e/*KEY_EXIT*/:		rccode = RC_HOME;		break;
...
int RenderChar(FT_ULong currentchar, int _sx, int _sy, int _ex, int color)
{
	...
	//
	// scale windows to real buffer
	if( var_screeninfo.xres < 1920)
	{
		if (_sx != -1)
			_sx = _sx * var_screeninfo.xres / (sx + ex);
		if (_ex != -1)
			_ex = _ex * var_screeninfo.xres / (sx + ex);
		if (_sy != -1)
			_sy = _sy * var_screeninfo.yres / (sy + ey);
		//if (ey != -1)
		//	ey = ey * var_screeninfo.yres / (sy + ey);
		//
	}
...
void plugin_exec(PluginParam * par)
{
	...
	// scale font to real buffer
	FONTHEIGHT_VERY_SMALL = _FONTHEIGHT_VERY_SMALL * var_screeninfo.yres / (sy + ey);
	FONTHEIGHT_SMALL = _FONTHEIGHT_SMALL * var_screeninfo.yres / (sy + ey);
	FONTHEIGHT_BIG = _FONTHEIGHT_BIG * var_screeninfo.yres / (sy + ey);
	FONT_OFFSET = _FONT_OFFSET * var_screeninfo.yres / (sy + ey);
	FONT_OFFSET_BIG = _FONT_OFFSET_BIG * var_screeninfo.yres / (sy + ey);
	MENUSIZE = _MENUSIZE * var_screeninfo.yres / (sy + ey);
	MINBOX = _MINBOX * var_screeninfo.yres / (sy + ey) /*720*/;
	BUTTONWIDTH = _BUTTONWIDTH * var_screeninfo.yres / (sy + ey);
	BUTTONHEIGHT = _BUTTONHEIGHT * var_screeninfo.yres / (sy + ey);
	
	// scale windows to real buffer
	if (sx != -1)
		sx = sx * var_screeninfo.xres / (sx + ex);
	if (ex != -1)
		ex = ex * var_screeninfo.xres / (sx + ex);
	if (sy != -1)
		sy = sy * var_screeninfo.yres / (sy + ey);
	if (ey != -1)
		ey = ey * var_screeninfo.yres / (sy + ey);
