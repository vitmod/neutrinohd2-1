...
void CPictureViewerGui::paintFoot()
{
	...
	{
		//OK
		frameBuffer->paintIcon(NEUTRINO_ICON_BUTTON_OKAY, x + ButtonWidth2 + 25, y + (height - buttonHeight) - 3);
		g_Font[SNeutrinoSettings::FONT_TYPE_INFOBAR_SMALL]->RenderString(x + ButtonWidth2 + 53 , y + (height - buttonHeight) + 24 - 4, ButtonWidth2 - 28, g_Locale->getText(LOCALE_PICTUREVIEWER_SHOW), COL_INFOBAR, 0, true); // UTF-8

		frameBuffer->paintIcon(NEUTRINO_ICON_BUTTON_5, x + ButtonWidth + 25, y + (height - buttonHeight) );
		...
		g_Font[SNeutrinoSettings::FONT_TYPE_INFOBAR_SMALL]->RenderString(x + ButtonWidth + 53 , y + (height - buttonHeight) + 24 - 4, ButtonWidth2 - 28, tmp, COL_INFOBAR, 0, true); // UTF-8
