...
{
	dprintf(DEBUG_INFO, "CFileBrowser::readDir_vlc: %s\n", dirname.c_str());
	...
	if (!answer.empty() && httpres == 0)
	{
		...
		if (answer_parser != NULL) 
		{
			...
			if (element == NULL) 
			{
				dprintf(DEBUG_INFO, "CFileBrowser::readDir_vlc: Drive is not readable. Possibly no disc inserted\n");
				CFile file;
...
void CFileBrowser::paintFoot()
{
	...
	// foot
	frameBuffer->paintBoxRel(x, y + height - (2 * foheight ), width, (2 * foheight ), COL_INFOBAR_SHADOW_PLUS_1, RADIUS_MID, (g_settings.rounded_corners == ONLY_TOP) ? 0x0 : CORNER_BOTTOM, CFrameBuffer::PAINT_SHADING, 2);
